...
// Stripe Payment Links. Swap these four placeholders for the live links and the
// whole page is wired. Until then the buttons route to the free check.
const STRIPE = {
  audit: process.env.NEXT_PUBLIC_STRIPE_AUDIT || INDEXED,
  sprint: process.env.NEXT_PUBLIC_STRIPE_SPRINT || INDEXED,
  starter: process.env.NEXT_PUBLIC_STRIPE_STARTER || INDEXED,
  growth: process.env.NEXT_PUBLIC_STRIPE_GROWTH || INDEXED,
};
...
const STEPS = [
  { t: "Check", b: "Run the free 60-second check, or take the full Audit and Plan to see exactly where you stand." },
  { t: "Fix", b: "We sort the foundations in a one-time Sprint. Clear scope, one price, no lock-in." },
  { t: "Stay found", b: "Always-On keeps you in the answers as the engines change month to month." },
];
  ...
  return (
    <>
      <header className="nav">
        <div className="wrap nav-inner">
          ...
          <nav className="nav-links">
            <a className="nav-hide" href={INDEXED}>Free check</a>
      ...
      <section className="offer wrap">
        ...
        <a className="offer-entry r" href={STRIPE.audit} target="_blank" rel="noopener noreferrer">
          <div className="offer-entry-main">
            ...
            <p className="offer-entry-body">
              Your score across the four things engines judge, the exact reasons you are being left out, and a
              90-day plan you can run yourself or hand to us.
            </p>
            ...
            <div className="opts">
              <div className="opt">
                <div className="opt-head">
                  <span className="opt-name">Starter</span>
                  <span className="price"><span className="amount sm">&euro;390</span><span className="per">/mo</span></span>
                </div>
                <p>Monitoring across the main AI engines, monthly fixes, and a plain score report.</p>
                <a className="offer-buy" href={STRIPE.starter} target="_blank" rel="noopener noreferrer">Choose Starter <span className="arr" aria-hidden="true">→</span></a>
              </div>
              <div className="opt">
                <div className="opt-head">
                  <span className="opt-name">Growth</span>
                  <span className="price"><span className="amount sm">&euro;890</span><span className="per">/mo</span></span>
                </div>
                <p>Everything in Starter, plus active content and citation building and deeper reporting.</p>
                <a className="offer-buy" href={STRIPE.growth} target="_blank" rel="noopener noreferrer">Choose Growth <span className="arr" aria-hidden="true">→</span></a>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="cta reveal">
        ...
        <p>Run the free 60-second check, or book a call and we&rsquo;ll walk you through it.</p>
        <div className="cta-acts">
          <a className="btn-link" href={INDEXED} target="_blank" rel="noopener noreferrer"><span>Run the free check</span><span className="arr" aria-hidden="true">→</span></a>
          <a className="btn-link ghost" href={CALENDLY} target="_blank" rel="noopener noreferrer"><span>Book a call</span><span className="arr" aria-hidden="true">→</span></a>
        </div>
      </section>
        ...
        <div className="footer-wrap">
          ...
          <div className="footer-legal">
            <div className="footer-legal-links">
              <a href={`${SITE}/terms`}>Terms</a><span aria-hidden="true">•</span><a href={`${SITE}/privacy`}>Privacy</a><span className="footer-legal-spacer" aria-hidden="true"></span><span className="footer-legal-bullet" aria-hidden="true">•</span><a href="https://app.calibrestudio.co/" target="_blank" rel="noopener noreferrer">Login</a><span aria-hidden="true">•</span><a href={INDEXED}>Indexed</a><span aria-hidden="true">•</span><a href={PORTAL} target="_blank" rel="noopener noreferrer">Manage subscription</a>
            </div>
